# 路由器 CSI 接收範例 [[English]](./README.md)

（更多範例說明請參考上一層 `examples` 目錄下的 README。）

此範例透過 Ping 指令觸發路由器發送封包，藉此取得裝置與路由器之間的 CSI 資料。

## 使用說明
在設定與建置專案前，請先執行 `idf.py set-target <chip_name>` 指定正確的晶片目標。

### 硬體需求

* 一塊搭載 ESP32 / ESP32-S2 / ESP32-C3 SoC 的開發板（例如 ESP32-DevKitC、ESP-WROVER-KIT 等）
* 一條 USB 線供電與下載韌體
...
執行 `idf.py menuconfig` 開啟設定選單，可在此配置 Wi-Fi 或乙太網路。詳細步驟請參考 ESP-IDF 範例文件「[Establishing Wi-Fi or Ethernet Connection](https://github.com/espressif/esp-idf/tree/master/examples/protocols#establishing-wi-fi-or-ethernet-connection)」章節。

### 建置與燒錄

建置專案並燒錄到開發板，接著啟動監控工具觀察序列埠輸出：

```
idf.py -p PORT flash monitor
```

（請將 PORT 換成實際使用的序列埠名稱。）

（要離開序列埠監控請按 ``Ctrl-]``。）

完整的 ESP-IDF 建置與使用流程請參考官方《Getting Started》指南。
...
CSI_DATA,0,94:d9:b3:80:8c:81,-30,11,1,6,1,0,1,0,1,0,0,-93,0,13,2,2751923,0,67,0,128,1,"[67,48,4,0,0,0,0,0,0,0,5,0,20,1,20,1
```
